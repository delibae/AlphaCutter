...
if debug == 0:
    ...
    parser.add_argument("--tmp_folder", required=True, type=str)
    parser.add_argument("--csv_save_path", required=True, type=str)
    parser.add_argument("--except_seq", required=True, type=bool)
    args = parser.parse_args()

    loop_min_len = args.loop_min
    helix_min_len = args.helix_min
    domain_min_len = args.domain_min
    if all([loop_min_len == None, helix_min_len == None]):
        fragment_min_len = 0
...
def get_helix_loop_list(dssp_secondary_struc, pdb_df_groupby, contact_list, min_len):
    ...
    for idx, rows in pdb_df_groupby.iterrows():
        ...
        for num in range(min_len):
            ...
            if current_idx < pdb_df_groupby.index.max():
                current_secondary_struc = pdb_df_groupby.at[
                    current_idx, "secondary_struc"
                ]

                secondary_struc_list_consec.append(current_secondary_struc)

        # check if all elements in secondary_struc_list_consec are loop
        secondary_struc_list_consec_boo = [
            i == dssp_secondary_struc for i in secondary_struc_list_consec
        ]
        ...
        if all(
            [
                len(secondary_struc_list_consec_boo) > 0,
    ...
    loop_list = [
        i for (i, v) in zip(loop_list, loop_list_contact_boo) if v
    ]  # remove elements according to boolean list
...
for pdb in filepath_list:
    ...
    for chain_id, chain_df in pdb_df_dict.items():
        ...
        for key in list(dssp.keys()):
            ...
            chain_df_res_three = chain_df.loc[
            ...
            if chain_df_res_one != res_dssp:
                ...
                chain_df.loc[
                    chain_df["residue_number"] == res_num_dssp, "secondary_struc"
                ] = secondary_struc
        ...
        # groupby residue
        pdb_df_dict_groupby[chain_id] = chain_df.groupby("residue_number").agg(
    ...
    for chain_id1, chain_df1 in pdb_df_dict_groupby.items():
        ...
        contact_map_dict[chain_id1] = pd.DataFrame(
            0, index=res_list1, columns=res_list2_concat
        )
    ...
    for chain_id1, chain_df1 in pdb_df_dict_groupby.items():
        ...
        for chain_id2, chain_df2 in pdb_df_dict_groupby.items():
            ...
            for i in pcmap_dict["data"]:
                ...
                for j in i["partners"]:
                    res_contact_num_list.append(
                        (chain_id2 + j["resID"]).replace(" ", "")
                    )
    ...
    contact_map_list = [
        contact_map for chain_id, contact_map in contact_map_dict.items()
    ]
    ...
    for (chain_id, chain_df), (chain_id, contact_map) in zip(
        pdb_df_dict_groupby.items(), contact_map_dict.items()
    ):
        contact_list = []
        ...
        for idx, rows in chain_df.iterrows():
            ...
            if idx + local_contact_range > chain_df.index.max():
                drop_list = [
                    chain_id + str(i) for i in range(idx - local_contact_range, idx)
                ]

            elif idx - local_contact_range < chain_df.index.min():
                drop_list = [
                    chain_id + str(i) for i in range(idx, idx + local_contact_range)
                ]

            else:
                drop_list = [
                    chain_id + str(i)
                    for i in range(idx - local_contact_range, idx + local_contact_range)
                ]
    ...
    if loop_min_len == None:
        loop_list_dict = {
            chain_id: [] for chain_id, chain_df in pdb_df_dict_groupby.items()
        }
        loop_list_merged_dict = loop_list_dict

    else:
        loop_list_dict = {
            chain_id: get_helix_loop_list(
                "-", chain_df, contact_list_dict[chain_id], loop_min_len
            )
            for chain_id, chain_df in pdb_df_dict_groupby.items()
        }
        loop_list_merged_dict = {
            chain_id: merge_list(
                get_helix_loop_list(
                    "-", chain_df, contact_list_dict[chain_id], loop_min_len
                ),
    ...
    if helix_min_len == None:
        ...
        helix_list_dict = {
            chain_id: get_helix_loop_list(
                "H", chain_df, contact_list_dict[chain_id], helix_min_len
            )
            for chain_id, chain_df in pdb_df_dict_groupby.items()
        }
        helix_list_merged_dict = {
            chain_id: merge_list(
                get_helix_loop_list(
                    "H", chain_df, contact_list_dict[chain_id], helix_min_len
                ),
    ...
    loop_helix_list_merged_dict = {
        chain_id: sorted(loop_list + helix_list)
        for (chain_id, loop_list), (chain_id, helix_list) in zip(
            loop_list_merged_dict.items(), helix_list_merged_dict.items()
        )
    }

    domain_list_dict = {}

    for (chain_id, chain_df), (chain_id, loop_helix_list_merged) in zip(
        pdb_df_dict_groupby.items(), loop_helix_list_merged_dict.items()
    ):
        ...
        domain_index_list = chain_df.loc[
            chain_df["loop_helix_boo"] == False
        ].index.tolist()
        ...
        if len(domain_list) > 0:
            if all([loop_min_len == None, helix_min_len == None]):
            ...
            elif all([loop_min_len == None, helix_min_len != None]):
                ...
                if any(
                    [
                        chain_df.index.max() - domain_list[-1][-1] < loop_min_len,
    ...
    domain_dict_keys = [
        k
        for chain_id, domain_dict in domain_dict_dict.items()
        for k, v in domain_dict.items()
    ]

    domain_key_contact_df = pd.DataFrame(
        0, index=domain_dict_keys, columns=domain_dict_keys
    )
            ...
            for k1, v1 in domain_dict1.items():
                for k2, v2 in domain_dict2.items():
                    if v1 != v2:
                        ...
                        domain1_domain2_contact_map_df = contact_map_df.loc[
                            chain_id1 + str(v1[0]) : chain_id1 + str(v1[1]),
                            chain_id2 + str(v2[0]) : chain_id2 + str(v2[1]),
                        ]
    ...
    # domain to range
    domain_dict_dict_format = {
        domain: [chain_id + str(range[0]), chain_id + str(range[1])]
        for chain_id, domain_dict in domain_dict_dict.items()
        for domain, range in domain_dict.items()
    }
    ...
    for merged_domain in domain_list_final_dict:
        ...
        for chain_id, domain in merged_domain.items():
            if len(domain) > 0:
                for idx in range(0, len(domain), 2):
                    ...
                    domain_df_cut = domain_df_full.loc[
                        (domain_df_full["residue_number"] >= int(domain[idx]))
                        & (domain_df_full["residue_number"] <= int(domain[idx + 1]))
                    ]

                    # domain cut length
                    domain_df_cut_chain_res_unique = domain_df_cut[
        ...
        if len(domain_df_list) > 0:
            ...
            # domain length
            domain_df_chain_res_unique = domain_df[
                ["chain_id", "residue_number"]
            ].drop_duplicates()
            ...
            if all([domain_pLDDT_mean >= pLDDT_min, domain_len >= domain_min_len]):
                ...
                for chain_id in domain_df_chain_res_unique["chain_id"].unique():
                    start = domain_df_chain_res_unique.loc[
                        domain_df_chain_res_unique["chain_id"] == chain_id
                    ]["residue_number"].min()
                    end = domain_df_chain_res_unique.loc[
                        domain_df_chain_res_unique["chain_id"] == chain_id
                    ]["residue_number"].max()
    ...
    if all([single_out == True, len(domain_df_list_single) > 0]):
        # merge single PDB output
        domain_df_single = pd.concat(domain_df_list_single)

        # domain length
        domain_df_single_chain_res_unique = domain_df_single[
            ["chain_id", "residue_number"]
        ].drop_duplicates()
        ...
        for chain_id in domain_df_single_chain_res_unique["chain_id"].unique():
            start = domain_df_single_chain_res_unique.loc[
                domain_df_single_chain_res_unique["chain_id"] == chain_id
            ]["residue_number"].min()
            end = domain_df_single_chain_res_unique.loc[
                domain_df_single_chain_res_unique["chain_id"] == chain_id
            ]["residue_number"].max()
            output_range_list.append(chain_id + str(start) + "-" + str(end))
        ...
        output_range = "_".join(output_range_list)
        ppdb.to_pdb(
            path=pdb.replace(".pdb", "") + "_AFCT-OUT_single_" + output_range + ".pdb",
            records=None,
            gz=False,
            append_newline=True,
        )
    ...
    for chain_id, loop_list_merged in loop_list_merged_dict.items():
        if len(loop_list_merged) > 0:
            for loop in loop_list_merged:
                loop_df = pdb_df.loc[
                    (pdb_df["residue_number"] >= loop[0])
                    & (pdb_df["residue_number"] <= loop[1])
                ]
    ...
    for chain_id, helix_list_merged in helix_list_merged_dict.items():
        if len(helix_list_merged) > 0:
            for helix in helix_list_merged:
                helix_df = pdb_df.loc[
                    (pdb_df["residue_number"] >= helix[0])
                    & (pdb_df["residue_number"] <= helix[1])
                ]
    ...
    for chain_id, domain_list in domain_list_dict.items():
        if len(domain_list) > 0:
            for domain in domain_list:
                domain_df = pdb_df.loc[
                    (pdb_df["residue_number"] >= domain[0])
                    & (pdb_df["residue_number"] <= domain[1])
                ]
    ...
    summary_df = pd.concat([domain_start_end_df, helix_start_end_df, loop_start_end_df])

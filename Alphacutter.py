...
def main_pr(
    ...
    loop_min_len = loop_min
    helix_min_len = helix_min
    domain_min_len = domain_min
    if all([loop_min_len == None, helix_min_len == None]):
        fragment_min_len = 0
        local_contact_range = 0
    else:
        fragment_min_len = fragment_min
        local_contact_range = local_contact_range
    pLDDT_min = pLDDT_min
    domain_out = domain_out
    single_out = single_out
    all_pdb = os.path.join(tmp_folder, "*.pdb")
    ...
    for pdb in filepath_list:
            ...
            for idx, rows in chain_df.iterrows():
                ...
                if idx + local_contact_range > chain_df.index.max():
                    ...
                    drop_list = [
                        chain_id + str(i)
                        for i in range(
                            idx - local_contact_range, idx + local_contact_range
                        )
                    ]
        ...
        summary_df = pd.concat(
            [domain_start_end_df, helix_start_end_df, loop_start_end_df]
        )
    ...
    # save summary
    summary_df_all.to_csv(os.path.join(csv_save_path, "AFCT-OUT_summary.csv"))
